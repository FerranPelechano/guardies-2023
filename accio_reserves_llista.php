...
if($filtro_aula!=""){
	$filtro=" AND R.AULA='".$filtro_aula."' ";
	$filtro_text=" (".$filtro_aula.") ";
	$filtro_text.="<a href=\"index.php?accio=reserves_llista\" class=\"btn btn-outline-dark btn-sm\" role=\"button\" aria-disabled=\"true\">X</a> ";
}
$filtro_profe=$_GET['filtro_profe'];
if($filtro_profe!=""){
	$filtro=" AND P.NOM='".$filtro_profe."' ";
...
<div class="container-fluid">     
	...
	<table class="table table-sm table-bordered table-striped">
		...
		<tbody>	
			...
			//Consulta
			$reserves=$db->query("
			SELECT  R.ID, R.IDPROFESSOR, R.DATA,R.HORA,R.AULA, R.OBSERVACIONS,P.NOM,".$horasql." FROM Reserves R LEFT JOIN Professor P ON R.IDPROFESSOR=P.ID WHERE DATA>='".date("Y-m-d" )."' ".$filtro." ORDER BY  DATA, HORAREAL, AULA")->fetchAll();						
			foreach($reserves as $reserva){      
				...
				echo "<td>";
				echo "<a href=\"index.php?accio=reserves_llista&filtro_aula=".$reserva_aula."\" class=\"btn btn-outline-dark btn-sm\" role=\"button\" aria-disabled=\"true\">F</a> ";
				echo $reserva_aula;
				echo"</td>";
				echo "<td>".$reserva_data."</td>";
				echo "<td>".$config_hores[$reserva_hora]."</td>";                
                echo "<td>";				
				echo "<a href=\"index.php?accio=reserves_llista&filtro_profe=".$reserva_profe."\" class=\"btn btn-outline-dark btn-sm\" role=\"button\" aria-disabled=\"true\">F</a> ";
				echo $reserva_profe;
				echo "</td>";				
                echo "<td>".$reserva_observacions."</td>";
				if ($username==$usuari_privilegiat){
					echo "<td>";
					echo "<a href=\"index.php?accio=reserves_elimina&id=".$reserva_id."&filtro_profe=".$filtro_profe."&filtro_aula=".$filtro_aula."\" class=\"btn btn-outline-danger btn-sm\" role=\"button\" aria-disabled=\"true\">".RESERVES_ELIMINA."</a>";
					echo "</td>";
				}
				echo "</tr>";
			}
